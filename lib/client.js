...
/**
	...
	* @constructor
	* @param {string} service - service url eg 'https://nuntium.instedd.org'
	...
	*/
function Client(service,account, application, password) {
...
Client.prototype = {
		...
		getCountries: function (callback) {
				this._get('/api/countries.json', callback);
		},
		/**
		...
		getCountry:   function (country_id, callback) {
			 if (country_id == null) throw new NuntiumError("country_id is required");
				this._get('/api/countries/' + country_id + '.json', callback);
		},
		/**
			* @param {string} [country_id]
			* @param {Nuntium~requestCallback} callback
		...
		getCarrier:   function (carrier_id, callback) {
				...
				this._get('/api/carriers/' + carrier_id + '.json', callback);
		},
		/**
		...
		getChannels:  function (callback) {
				this._get('/api/channels.json', callback);
		},
		/**
		...
		getChannel:   function (name, callback) {
				if (name == null) throw new NuntiumError("name of the channel is required");
				this._get('/api/channels/' + name + '.json', callback);
		},
		/**
			*
			* @param name
		...
		getAO: function(token,callback) {
				...
				this._get(url.format(uri), callback);
		},
		/**
			*
		...
		createTwitterFriendship: function(args) {
				...
				this._get(url.format(uri), function() {
		...
		_get:     function (uri, callback) {
				rest.get(this.service+uri,this.defaultOptions).on('complete', callback);
		},
		_postJson:     function (uri, attributes, options, callback) {
				rest.postJson(this.service+url.format(uri),attributes,this.defaultOptions).on('complete', callback);
		},
		_put:     function (uri, attributes, callback) {
				rest.putJson(this.service+url.format(uri),attributes,this.defaultOptions).on('complete', callback);
		},
		_del:   function (uri, callback) {
				rest.del(this.service+url.format(uri),this.defaultOptions).on('complete', callback);
		},
		_sendOneAO: function(args,callback) {
				var uri = {
						"pathname": '/'+this.account+'/' + this.application +'/'+'send_ao',
						"query": args
				};

				this._get(url.format(uri),function(data,response) {
						// TODO: deal with missing headers
						// TODO: DRY this
						var nuntiumHeaders= {
							'id': response.headers['x-nuntium-id'],
							'guid': response.headers['x-nuntium-guid'],
							'token': response.headers['x-nuntium-token']
						};
						callback(nuntiumHeaders,response);
				});
		},
